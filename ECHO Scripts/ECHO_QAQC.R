...
ECHO_Discharge<-read.table("G:/My Drive/ECHO NPDES/USGS_Consumptive_Use_Updated/Documentation/Imports/ECHO_timeseries_3_7.txt",header=T, sep="\t")
  ...
  for(i in 1:length(Fac_Type_VPDES$Facility.ID)){
    ...
    if(is.na(Fac_Type_VPDES$SIC[i])){
      Fac_Type_VPDES$Reclass_Use_Type[i]<-NA
       ...
       substr(Fac_Type_VPDES$SIC[i],0,2)=="02"| # Livestock
       substr(Fac_Type_VPDES$SIC[i],0,2)=="07"| # Agricultural Services
       substr(Fac_Type_VPDES$SIC[i],0,2)=="08"| # Forestry
       substr(Fac_Type_VPDES$SIC[i],0,2)=="09"| # Fishing, Hunting and Trapping
...
Rule_Based<- function(){
  ...
  Fac_Type_VPDES_m<-subset(Fac_Type_VPDES,select = c("Facility.ID","Reclass_Use_Type"))
  ...
  change_use("VA0091278","Agriculture/Irrigation")
  change_use("VA0091260","Agriculture/Irrigation")
...
Facility_Discharges%>%dplyr::group_by(Year)%>%

...
Ave_NWB_CU<- function(VA_Counties,label){
  ...
  for (i in 1:length(VA_Counties@data$TNMID)){
  ...
  for (i in 1:nrow(VA_Counties@data)){
    VA_Counties@data$Discharge_ave[i]<-VA_Counties@data$Discharge_sum[i]/
      length(which(!is.na(VA_Counties@data[i,c("Discharges_2010","Discharges_2011","Discharges_2012",
  ...
  for (i in 1:nrow(VA_Counties@data)){
    VA_Counties@data$Withdrawal_ave[i]<-VA_Counties@data$Withdrawal_sum[i]/
      length(which(!is.na(VA_Counties@data[i,c("Withdrawals_2010","Withdrawals_2011","Withdrawals_2012",
  ...
  for (i in 1:nrow(VA_Counties@data)){
    VA_Counties@data$NetWB_ave[i]<-VA_Counties@data$NetWB_sum[i]/
      length(which(!is.na(VA_Counties@data[i,c("NetWB_2010","NetWB_2011","NetWB_2012",
  ...
  for (i in 1:nrow(VA_Counties@data)){
    VA_Counties@data$Consumption_ave[i]<-VA_Counties@data$Consumption_sum[i]/
      length(which(!is.na(VA_Counties@data[i,c("Consumption_2010","Consumption_2011","Consumption_2012",
  ...
  for (i in 1:nrow(VA_Counties@data)){
    VA_Counties@data$NetWB_t_ave[i]<-VA_Counties@data$NetWB_t_sum[i]/
      length(which(!is.na(VA_Counties@data[i,c("NetWB_2010_t","NetWB_2011_t","NetWB_2012_t",
  ...
  for (i in 1:nrow(VA_Counties@data)){
    VA_Counties@data$Consumption_t_ave[i]<-VA_Counties@data$Consumption_t_sum[i]/
      length(which(!is.na(VA_Counties@data[i,c("Consumption_2010_t","Consumption_2011_t","Consumption_2012_t",
  ...
  VA_Counties_glimpse<-VA_Counties@data
  VA_Counties_glimpse[15:98]<-sapply(VA_Counties_glimpse[15:98],as.numeric)
  VA_Counties_glimpse[15:98]<<-round(VA_Counties_glimpse[15:98], digits=2)
...
county_discharge<- function(VA_Counties,ECHO_points, label,fileext){
...
#-All Facilities-#
county_discharge(VA_Counties,ECHO.test,"All Reporting Facilities in All Sectors","all_fac_all_sec")
#-Fully Matched Facilities-#
...
county_discharge(VA_Counties_ag,ECHO.test_ag,"Agriculture/Irrigation Facilities","all_ag")
...
county_discharge(VA_Counties_match_ag,ECHO.test_match_ag,"Matched Agriculture/Irrigation Facilities","match_ag")
...
county_discharge(VA_Counties_match_municipal,ECHO.test_match_municipal,"Matched Municipal Facilities","match_municipal")
...
county_points<- function(){
  ...
  plot1<-ggplot()+
    geom_polygon(data=VA_Counties.df,aes(x=long, y= lat, group=group, fill= "transparent", colour="#252525"))+
    geom_point(data=ECHO_points, aes(x=Fac.Long, y=Fac.Lat, shape="Outfall"), size=1.25,colour="#252525")+
    scale_size_manual(name="Summed Discharge (MGD)", values=(Dis_Discrete), 
                      labels=c(paste("<",1),
                               paste(1,"-",5),
                               paste(5,"-",10),
                               paste(10,"-",25),
...
county_withdrawal<- function(VA_Counties,VWUDS_points,label,fileext){
...
#-All Facilities-#
county_withdrawal(VA_Counties,VWUDS.test,"All Reporting Facilities in All Sectors","all_fac_all_sec")
#-Fully Matched Facilities-#
...
county_withdrawal(VA_Counties_match_ag,VWUDS.test_match_ag,"Matched Agriculture/Irrigation Facilities","match_ag")
...
county_withdrawal(VA_Counties_match_municipal,VWUDS.test_match_municipal,"Matched Municipal Facilities","match_municipal")
...
county_consumption<- function(VA_Counties, ECHO_points ,VWUDS_points, label,fileext){
  VA_Counties.df<-broom::tidy(VA_Counties)
  VA_Counties$polyID<-sapply(slot(VA_Counties,"polygons"), function(x) slot(x, "ID"))
  VA_Counties.df<-merge(VA_Counties.df, VA_Counties, by.x="id", by.y="polyID")
...
#-Fully Matched Facilities-#
county_consumption(VA_Counties_matched,ECHO.test_matched,VWUDS.test_matched,"Matched Facilities in All Sectors","match_all_sector")

#---Subsetted by Sector---#

#-All Facilities-#
county_consumption(VA_Counties_energy,ECHO.test_energy,VWUDS.test_energy,"Energy Facilities","all_energy")
county_consumption(VA_Counties_ag,ECHO.test_ag,VWUDS.test_ag,"Agriculture/Irrigation Facilities","all_ag")
...
county_consumption(VA_Counties_match_ag,ECHO.test_match_ag,VWUDS.test_match_ag,"Matched Agriculture/Irrigation Facilities","match_ag")
...
county_consumption(VA_Counties_match_municipal,ECHO.test_match_municipal,VWUDS.test_match_municipal,"Matched Municipal Facilities","match_municipal")

#---Non-Energy Sectors---#

#-All Facilities-#
county_consumption(VA_Counties_nonenergy,ECHO.test_nonenergy,VWUDS.test_nonenergy,"Non-Energy Facilities","all_nonenergy")

#-Fully Matched Facilities-#
county_consumption(VA_Counties_match_nonenergy,ECHO.test_match_nonenergy,VWUDS.test_match_nonenergy,"Matched Non-Energy Facilities","match_nonenergy")

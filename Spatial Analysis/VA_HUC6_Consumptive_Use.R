...
Ave_NWB_CU<- function(HUC6,label){
  ...
  for (i in 1:nrow(HUC6@data)){
    HUC6@data$Discharge_ave[i]<-HUC6@data$Discharge_sum[i]/
      length(which(!is.na(HUC6@data[i,c("Discharges_2010","Discharges_2011","Discharges_2012",
  ...
  for (i in 1:nrow(HUC6@data)){
    HUC6@data$Withdrawal_ave[i]<-HUC6@data$Withdrawal_sum[i]/
      length(which(!is.na(HUC6@data[i,c("Withdrawals_2010","Withdrawals_2011","Withdrawals_2012",
  ...
  for (i in 1:nrow(HUC6@data)){
    HUC6@data$NetWB_ave[i]<-HUC6@data$NetWB_sum[i]/
      length(which(!is.na(HUC6@data[i,c("NetWB_2010","NetWB_2011","NetWB_2012",
  ...
  for (i in 1:nrow(HUC6@data)){
    HUC6@data$Consumption_ave[i]<-HUC6@data$Consumption_sum[i]/
      length(which(!is.na(HUC6@data[i,c("Consumption_2010","Consumption_2011","Consumption_2012",
  ...
  for (i in 1:nrow(HUC6@data)){
    HUC6@data$NetWB_t_ave[i]<-HUC6@data$NetWB_t_sum[i]/
      length(which(!is.na(HUC6@data[i,c("NetWB_2010_t","NetWB_2011_t","NetWB_2012_t",
  ...
  for (i in 1:nrow(HUC6@data)){
    HUC6@data$Consumption_t_ave[i]<-HUC6@data$Consumption_t_sum[i]/
      length(which(!is.na(HUC6@data[i,c("Consumption_2010_t","Consumption_2011_t","Consumption_2012_t",
...
HUC6_labels<- function(HUC6){
  ...
  HUC6_Centroids<-as.data.frame(coordinates(HUC6_Clipped))
  names(HUC6_Centroids)<-c("Longitude","Latitude")
  HUC6_Centroids$HUC6<-HUC6_Clipped@data$HUC6
  HUC6_Names<-subset(HUC6_Clipped@data,select=c(11,12))
  HUC6_Centroids<-merge(HUC6_Centroids,HUC6_Names,by="HUC6")
  
  ggplot()+
    geom_polygon(data=VA_Counties,aes(x=long,y=lat,group=group), colour="grey",alpha=0.5,fill="transparent")+
    geom_polygon(data=HUC6_Clipped,aes(x=long, y= lat, group=group), colour='black', fill=NA)+
...
HUC6_discharge<- function(HUC6,ECHO_points, label,fileext){
...
#-All Facilities-#
HUC6_discharge(HUC6,ECHO.test,"All Reporting Facilities in All Sectors","all_fac_all_sec")
#-Fully Matched Facilities-#
...
HUC6_discharge(HUC6_ag,ECHO.test_ag,"Agriculture/Irrigation Facilities","all_ag")
...
HUC6_discharge(HUC6_match_ag,ECHO.test_match_ag,"Matched Agriculture/Irrigation Facilities","match_ag")
HUC6_discharge(HUC6_match_commercial,ECHO.test_match_commercial,"Matched Commercial Facilities","macth_commercial")
HUC6_discharge(HUC6_match_industrial,ECHO.test_match_industrial,"Matched Industrial Facilities","match_industrial")
HUC6_discharge(HUC6_match_municipal,ECHO.test_match_municipal,"Matched Municipal Facilities","match_municipal")
...
HUC6_withdrawal<- function(HUC6,VWUDS_points,label,fileext){
...
#-All Facilities-#
HUC6_withdrawal(HUC6,VWUDS.test,"All Reporting Facilities in All Sectors","all_fac_all_sec")
#-Fully Matched Facilities-#
...
HUC6_withdrawal(HUC6_match_ag,VWUDS.test_match_ag,"Matched Agriculture/Irrigation Facilities","match_ag")
...
HUC6_withdrawal(HUC6_match_municipal,VWUDS.test_match_municipal,"Matched Municipal Facilities","match_municipal")
...
HUC6_consumption<- function(HUC6, ECHO_points ,VWUDS_points, label,fileext){
...
HUC6_consumption(HUC6,ECHO.test,VWUDS.test,"All Reporting Facilities in All Sectors","all_fac_all_sector")

#-Fully Matched Facilities-#

HUC6_consumption(HUC6_matched,ECHO.test_matched,VWUDS.test_matched,"Matched Facilities in All Sectors","match_all_sector")

#---Subsetted by Sector---#

#-All Facilities-#
HUC6_consumption(HUC6_energy,ECHO.test_energy,VWUDS.test_energy,"Energy Facilities","all_energy")
HUC6_consumption(HUC6_ag,ECHO.test_ag,VWUDS.test_ag,"Agriculture/Irrigation Facilities","all_ag")
...
HUC6_consumption(HUC6_match_ag,ECHO.test_match_ag,VWUDS.test_match_ag,"Matched Agriculture/Irrigation Facilities","match_ag")
...
HUC6_consumption(HUC6_match_municipal,ECHO.test_match_municipal,VWUDS.test_match_municipal,"Matched Municipal Facilities","match_municipal")

#---Non-Energy Sectors---#

#-All Facilities-#
HUC6_consumption(HUC6_nonenergy,ECHO.test_nonenergy,VWUDS.test_nonenergy,"Non-Energy Facilities","all_nonenergy")

#-Fully Matched Facilities-#
HUC6_consumption(HUC6_match_nonenergy,ECHO.test_match_nonenergy,VWUDS.test_match_nonenergy,"Matched Non-Energy Facilities","match_nonenergy")

...
Match_ECHO_2010_2017<-subset(ECHO_2010_2017,ECHO_2010_2017$Facility.ID%in%gsub("echo_","",Matched$VPDES.Hydrocode), select=c(1,3:9,12,19,33,43,44))
...
ECHO_2010_2017<-subset(ECHO_2010_2017,select=c(1:9,10,12,19,33,40,43,44,45))
...
full_match_2010_2017<-full_match_2010_2017%>%dplyr::group_by(VWUDS.Facility.ID,VPDES.Facility.ID,
                                       ...
                                       County,Waterbody)%>%dplyr::summarise(Discharges_MGD=sum(Discharges_MGD,na.rm=T),n_outfalls=n_distinct(OutfallID))
...
Cum_Withdrawal<- function(Facility_Withdrawals,label){
  ...
  Long_term_ave<-median(Yearly_Withdrawal$Withdrawals_MGD)
...
path="G:/My Drive/USGS_ConsumptiveUse/Spring, 2019/Statewide Analysis/"
file_name <- paste(path,"Matched_vs_Total_With_",Sector, ".pdf", sep="")
...
# This for loop goes through each facility to speed up getting results. Major water users are at top of list
  ...
  print(list(full_match_2010_2017_summary$VWUDS.Facility.ID[i],
        full_match_2010_2017_summary$Use.Type[i],
        full_match_2010_2017_summary$Waterbody[i],
        Withdrawal$perc_total[match(full_match_2010_2017_summary$VWUDS.Facility.ID[i],Withdrawal$VWUDS.Facility.ID)]*100,
        Discharge$perc_total[match(full_match_2010_2017_summary$VPDES.Facility.ID[i],gsub("echo_","",Discharge$VPDES.Facility.ID))]*100))
...
path="G:/My Drive/USGS_ConsumptiveUse/Spring, 2019/Matched Facilities/Case Studies/Fully_Matched/"
# save plots to PNG. Makes a separate file for each plot
for (i in 1:length(full_match_2010_2017_summary$VPDES.Facility.ID)) {
  file_name <- paste("Outfall_compare_", i, ".png", sep="")
  png(file=paste0(path,file_name),width=1000,height=500)
  print(plot_list[[i]])
  dev.off()
}
...
TS_Consumption<- function(Discharge,Withdrawal,name){
  ...
  Ave_CU<-df%>%dplyr::group_by(Year=substr(Date,1,4))%>%dplyr::summarise(Med_CU=median(Consumption,na.rm=T),Ave_CU=mean(Consumption,na.rm=T))
